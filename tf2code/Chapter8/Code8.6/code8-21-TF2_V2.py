...
def _loop_train(model,optimizer, generator,grad):
    ...
    for _ in tqdm(range(n_steps)):#按批次循环获取数据，并计算训练
        ...
        optimizer.apply_gradients(zip(grad(yolo_v3,xs, ys),yolo_v3.trainable_variables))
'''
...
for i in range(num_epoches):
    ...
    for _ in tqdm(range(n_steps)):#按批次循环获取数据，并计算训练
        ...
        yolo_3=tf.convert_to_tensor(value=yolo_3)
        ys = [yolo_1, yolo_2, yolo_3]
        grad=_grad_fn(yolo_v3,xs,ys)
        #optimizer.apply_gradients(zip(grad,yolo_v3.trainable_variables))
        optimizer.apply_gradients(zip(grad,yolo_v3.variables))
