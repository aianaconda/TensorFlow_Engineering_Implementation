...
predata = __import__("code8-6-TF2")
mydataset = predata.mydataset
text_cnn = __import__("code8-7-TF2")
...
def train():
    ...
    with tf.compat.v1.Session(config=session_conf) as sess:
        ...
        i = 0
        while  tf.compat.v1.train.global_step(sess, cnn.global_step) < num_steps:
            x_batch, y_batch = sess.run(next_element)
            i = i+1
            time_str, step, loss, accuracy =train_step(x_batch, y_batch)
            
            current_step = tf.compat.v1.train.global_step(sess, cnn.global_step)
