...
features = tf.placeholder("float",[None])#重新定义占位符
...
loss = tf.losses.mean_squared_error(labels=labels, predictions=predictions)#定义损失函数

global_step = tf.train.get_or_create_global_step()#重新定义global_step

optimizer = tf.train.AdagradOptimizer(learning_rate=0.1)
train_op = optimizer.minimize(loss, global_step=global_step)

saver = tf.train.Saver(tf.global_variables(), max_to_keep=1)#重新定义saver
...
one_element = dataset.make_one_shot_iterator().get_next() #获得输入数据源张量  
