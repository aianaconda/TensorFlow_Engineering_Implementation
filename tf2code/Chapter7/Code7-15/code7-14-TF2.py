...
class CrossCompressUnit(base.Layer):
    def __init__(self, dim, name=None):
       ...
       self.dim = dim
       self.f_vv = tf.compat.v1.layers.Dense(1, use_bias = False)
       self.f_ev = tf.compat.v1.layers.Dense(1, use_bias = False)
       self.f_ve = tf.compat.v1.layers.Dense(1, use_bias = False)
       self.f_ee = tf.compat.v1.layers.Dense(1, use_bias = False)
       self.bias_v = self.add_weight(name='bias_v', shape=dim, initializer=tf.compat.v1.zeros_initializer())
       self.bias_e = self.add_weight(name='bias_e', shape=dim, initializer=tf.compat.v1.zeros_initializer())

    def __call__(self, inputs):
        v, e = inputs#v和e的形状为[batch_size, dim]
        v = tf.expand_dims(v, axis=2)#v的形状为 [batch_size, dim, 1]
        e = tf.expand_dims(e, axis=1)#e的形状为 [batch_size, 1, dim]

        c_matrix = tf.matmul(v, e)#c_matrix的形状为 [batch_size, dim, dim]
        c_matrix_transpose = tf.transpose(a=c_matrix, perm=[0, 2, 1])
...
class MKR(object):
    ...
    def _build_inputs(self):
        self.user_indices = tf.compat.v1.placeholder(tf.int32, [None], 'user_indices')
        self.item_indices = tf.compat.v1.placeholder(tf.int32, [None], 'item_indices')
        self.labels = tf.compat.v1.placeholder(tf.float32, [None], 'labels')
        self.head_indices = tf.compat.v1.placeholder(tf.int32, [None], 'head_indices')
        self.tail_indices = tf.compat.v1.placeholder(tf.int32, [None], 'tail_indices')
        self.relation_indices = tf.compat.v1.placeholder(tf.int32, [None], 'relation_indices')
        

    def _build_low_layers(self, args):
        #生成词嵌入
        self.user_emb_matrix = tf.compat.v1.get_variable('user_emb_matrix', [self.n_user, args.dim])
        self.item_emb_matrix = tf.compat.v1.get_variable('item_emb_matrix', [self.n_item, args.dim])
        self.entity_emb_matrix = tf.compat.v1.get_variable('entity_emb_matrix', [self.n_entity, args.dim])
        self.relation_emb_matrix = tf.compat.v1.get_variable('relation_emb_matrix', [self.n_relation, args.dim])
        ...
        self.item_embeddings = tf.nn.embedding_lookup(params=self.item_emb_matrix, ids=self.item_indices)
        self.head_embeddings = tf.nn.embedding_lookup(params=self.entity_emb_matrix, ids=self.head_indices)
        self.relation_embeddings = tf.nn.embedding_lookup(params=self.relation_emb_matrix, ids=self.relation_indices)
        self.tail_embeddings = tf.nn.embedding_lookup(params=self.entity_emb_matrix, ids=self.tail_indices)

        for _ in range(args.L):#按指定参数，构建多层MKR结构
            #定义全连接
            user_mlp = tf.compat.v1.layers.Dense(args.dim, activation=tf.nn.relu)
            tail_mlp = tf.compat.v1.layers.Dense(args.dim, activation=tf.nn.relu)
    ...
    def _build_high_layers(self, args):
        ...
        if use_inner_product:    #内积方式
            ...
            for _ in range(args.H - 1):
                rs_mlp = tf.compat.v1.layers.Dense(args.dim * 2, activation=tf.nn.relu)
                #self.user_item_concat的形状为[batch_size, dim * 2]
                self.user_item_concat = rs_mlp(self.user_item_concat)
                self.vars_rs.extend(rs_mlp.variables)
            #定义全连接
            rs_pred_mlp = tf.compat.v1.layers.Dense(1, activation=tf.nn.relu)
            #self.scores的形状为[batch_size]
        ...
        for _ in range(args.H - 1):
            kge_mlp = tf.compat.v1.layers.Dense(args.dim * 2, activation=tf.nn.relu)
            self.head_relation_concat = kge_mlp(self.head_relation_concat)
            self.vars_kge.extend(kge_mlp.variables)

        kge_pred_mlp = tf.compat.v1.layers.Dense(args.dim, activation=tf.nn.relu)
        ...
        self.scores_kge = tf.nn.sigmoid(tf.reduce_sum(input_tensor=self.tail_embeddings * self.tail_pred, axis=1))
        self.rmse = tf.reduce_mean(
            input_tensor=tf.sqrt(tf.reduce_sum(input_tensor=tf.square(self.tail_embeddings - self.tail_pred), axis=1) / args.dim))

    def _build_loss(self, args):
        #计算RS模型的loss值
        self.base_loss_rs = tf.reduce_mean(
            input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=self.labels, logits=self.scores))
    ...
    def _build_train(self, args):
        self.optimizer_rs = tf.compat.v1.train.AdamOptimizer(args.lr_rs).minimize(self.loss_rs)
        self.optimizer_kge = tf.compat.v1.train.AdamOptimizer(args.lr_kge).minimize(self.loss_kge)
