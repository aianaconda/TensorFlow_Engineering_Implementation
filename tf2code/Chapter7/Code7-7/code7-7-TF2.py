...
categorical_column_a = tf.feature_column.sequence_categorical_column_with_identity( #带序列的离散列
...
shared_embedding_columns = tf.feature_column.shared_embeddings( #共享列
...
sequence_feature_layer = tf.keras.experimental.SequenceFeatures(feature_columns=[embedding_column_b, embedding_column_a])#定义序列输入层
input_layer, sequence_length = sequence_feature_layer(features)

sequence_feature_layer2=tf.keras.experimental.SequenceFeatures(feature_columns=shared_embedding_columns)
input_layer2, sequence_length2 = sequence_feature_layer2(features) #定义序列输入层

global_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES)#返回图中的张量（2个嵌入词权重）
