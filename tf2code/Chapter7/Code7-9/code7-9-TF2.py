...
def placeholder_inputs():
  ...
  """
  images_placeholder = tf.compat.v1.placeholder(tf.float32, shape=(None,
                                                         mnist.IMAGE_PIXELS))
...
def inference(inp, num_clusters, hidden1_units, hidden2_units):
  ...
  kmeans = tf.compat.v1.estimator.experimental.KMeans(
      inp,
      num_clusters,
      distance_metric=tf.contrib.factorization.COSINE_DISTANCE,#这句有问题
  ...
  clustering_loss = tf.reduce_sum(input_tensor=clustering_scores[0])
  # Hidden 1
  with tf.compat.v1.name_scope('hidden1'):
    weights = tf.Variable(
        tf.random.truncated_normal([num_clusters, hidden1_units],
