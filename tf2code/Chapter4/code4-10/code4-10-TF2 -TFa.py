...
def _random_rotated30(image, label): #定义函数实现图片随机旋转操作
    def _rotatedwrap():
        ...
        # image_rotated = tfa.image.rotate(image,np.deg2rad(30))   #调用第三方函数
        return tf.cast(image_rotated,tf.float32)

    a = tf.random.uniform([1],0,2,tf.int32)#实现随机功能
    image_decoded = tf.cond(pred=tf.math.equal(tf.constant(0),a[0]),true_fn=lambda: image, false_fn=_rotatedwrap)
...
print(tf.compat.v1.data.get_output_types(tdataset))
print(tf.compat.v1.data.get_output_shapes(tdataset))
