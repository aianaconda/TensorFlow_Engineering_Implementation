...
def read_data(file_queue):                          #csv文件处理函数
    reader = tf.compat.v1.TextLineReader(skip_header_lines=1)  #tf.TextLineReader 可以每次读取一行
    key, value = reader.read(file_queue)
    
    defaults = [[0], [0.], [0.], [0.], [0.], [0]]       #为每个字段设置初始值
    cvscolumn = tf.io.decode_csv(records=value, record_defaults=defaults)           #tf.decode_csv对每一行进行解析
...
def create_pipeline(filename, batch_size, num_epochs=None): #创建队列数据集函数
    ...
    feature_batch, label_batch = tf.compat.v1.train.shuffle_batch(    #生成乱序的批次数据
...
with tf.compat.v1.Session() as sess:
    ...
    local_init_op = tf.compat.v1.local_variables_initializer()            #初始化本地变量，没有回报错
    sess.run(init_op)
    sess.run(local_init_op)

    coord = tf.train.Coordinator()                          #创建协调器
    threads = tf.compat.v1.train.start_queue_runners(coord=coord)    #开启线程列队
