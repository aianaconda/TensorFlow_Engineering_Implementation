...
9.7  实例52：用TFTS框架预测某地区每天的出生人数
9.7.1  准备样本
9.7.2  代码实现：数据预处理——制作TFTS框架中的读取器
9.7.3  代码实现：用TFTS框架定义模型，并进行训练
9.7.4  代码实现：用TFTS框架评估模型
9.7.5  代码实现：用模型进行预测，并将结果可视化
9.7.6  运行程序
9.7.7  扩展：用TFTS框架进行异常值检测
9.8  实例53：用Tacotron模型合成中文语音（TTS）
9.8.1  准备安装包及样本数据
9.8.2  代码实现：将音频数据分帧并转为梅尔频谱
9.8.3  代码实现：用多进程预处理样本并保存结果
9.8.4  拆分Tacotron网络模型的结构
9.8.5  代码实现：搭建CBHG网络
9.8.6  代码实现：构建带有混合注意力机制的模块
9.8.7  代码实现：构建自定义wrapper
9.8.8  代码实现：构建自定义采样器
9.8.9  代码实现：构建自定义解码器
9.8.10  代码实现：构建输入数据集
9.8.11  代码实现：构建Tacotron网络
9.8.12  代码实现：构建Tacotron网络模型的训练部分
9.8.13  代码实现：训练模型并合成音频文件
9.8.14  扩展：用pypinyin模块实现文字到声音的转换
第4篇  高级
第10章
生成式模型——能够输出内容的模型
10.1  快速导读
...
10.1.5  什么是实例归一化
10.1.6  了解SwitchableNorm及更多的归一化方法
10.1.7  什么是图像风格转换任务
10.1.8  什么是人脸属性编辑任务
10.1.9  什么是TFgan框架
10.2  实例54：构建DeblurGAN模型，将模糊相片变清晰
10.2.1  获取样本
10.2.2  准备SwitchableNorm算法模块
10.2.3  代码实现：构建DeblurGAN中的生成器模型
10.2.4  代码实现：构建DeblurGAN中的判别器模型
10.2.5  代码实现：搭建DeblurGAN的完整结构
...
10.2.11  练习题
10.2.12  扩展：DeblurGAN模型的更多妙用
10.3  实例55：构建AttGAN模型，对照片进行加胡子、加头帘、加眼镜、变年轻等修改
10.3.1  获取样本
10.3.2  了解AttGAN模型的结构
10.3.3  代码实现：实现支持动态图和静态图的数据集工具类
10.3.4  代码实现：将CelebA做成数据集
10.3.5  代码实现：构建AttGAN模型的编码器
10.3.6  代码实现：构建含有转置卷积的解码器模型
10.3.7  代码实现：构建AttGAN模型的判别器模型部分
10.3.8  代码实现：定义模型参数，并构建AttGAN模型
10.3.9  代码实现：定义训练参数，搭建正反向模型
10.3.10  代码实现：训练模型
10.3.11  扩展实例56：为人脸添加不同的眼镜
10.3.12  扩展：AttGAN模型的局限性
10.4  实例57：用RNN.WGAN模型模拟生成恶意请求
10.4.1  获取样本：通过Panabit设备获取恶意请求样本
10.4.2  了解RNN.WGAN模型
10.4.3  代码实现：构建RNN.WGAN模型
10.4.4  代码实现：训练指定长度的RNN.WGAN模型
...
11.1  快速导读
11.1.1  什么是FGSM方法
11.1.2  什么是cleverhans模块
11.1.3  什么是黑箱攻击
11.1.4  什么是基于雅可比矩阵的数据增强方法
11.1.5  什么是数据中毒攻击
11.2  实例58：用FGSM方法生成样本，并攻击PNASNet模型，让其将“狗”识别成“盘子”
11.2.1  代码实现：创建PNASNet模型
11.2.2  代码实现：搭建输入层并载入图片，复现PNASNet模型的预测效果
11.2.3  代码实现：调整参数，定义图片的变化范围
11.2.4  代码实现：用梯度下降方式生成对抗样本
11.2.5  代码实现：用生成的样本攻击模型
11.2.6  扩展：如何防范这种攻击模型的行为
11.2.7  代码实现：将数据增强方式用在使用场景，以加固PNASNet模型，防范攻击
11.3  实例59：击破数据增强防护，制作抗旋转对抗样本
11.3.1  代码实现：对输入的数据进行多次旋转
11.3.2  代码实现：生成并保存鲁棒性更好的对抗样本
11.3.3  代码实现：在PNASNet模型下比较对抗样本的效果
11.4  实例60：以黑箱方式攻击未知模型
11.4.1  准备工程代码
...
第5篇  实战——深度学习实际应用
第12章
TensorFlow模型制作——一种功能，多种身份
12.1  快速导读
12.1.1  详细分析检查点文件
12.1.2  什么是模型中的冻结图
12.1.3  什么是TF Serving模块与saved_model模块
12.1.4  用编译子图（defun）提升动态图的执行效率
12.1.5  什么是TF_Lite模块
12.1.6  什么是TFjs-converter模块
12.2  实例61：在源码与检查点文件分离的情况下，对模型进行二次训练
12.2.1  代码实现：在线性回归模型中，向检查点文件中添加指定节点
12.2.2  代码实现：在脱离源码的情况下，用检查点文件进行二次训练
12.2.3  扩展：更通用的二次训练方法
12.3  实例62：导出/导入冻结图模型
12.3.1  熟悉TensorFlow中的freeze_graph工具脚本
12.3.2  代码实现：从线性回归模型中导出冻结图模型
12.3.3  代码实现：导入冻结图，并用模型进行预测
12.4  实例63：逆向分析冻结图模型
12.4.1  使用import_to_tensorboard工具
12.4.2  用TensorBoard工具查看模型结构
12.5  实例64：用saved_model模块导出与导入模型文件
12.5.1  代码实现：用saved_model模块导出模型文件
12.5.2  代码实现：用saved_model模块导入模型文件
12.5.3  扩展：用saved_model模块导出带有签名的模型文件
12.6  实例65：用saved_model_cli工具查看及使用saved_model模型
12.6.1  用show参数查看模型
12.6.2  用run参数运行模型
12.6.3  扩展：了解scan参数的黑名单机制
12.7  实例66：用TF-Hub库导入、导出词嵌入模型文件
12.7.1  代码实现：模拟生成通用词嵌入模型
12.7.2  代码实现：用TF-Hub库导出词嵌入模型
12.7.3  代码实现：导出TF-Hub模型
12.7.4  代码实现：用TF-Hub库导入并使用词嵌入模型
第13章
布署TensorFlow模型——模型与项目的深度结合
13.1  快速导读
13.1.1  什么是gRPC服务与HTTP/REST API
13.1.2  了解TensorFlow对移动终端的支持
13.1.3  了解树莓派上的人工智能
13.2  实例67：用TF_Serving布署模型并进行远程使用
13.2.1  在Linux系统中安装TF_Serving
13.2.2  在多平台中用Docker安装TF_Serving
13.2.3  编写代码：固定模型的签名信息
13.2.4  在Linux中开启TF_Serving服务
13.2.5  编写代码：用gRPC访问远程TF_Serving服务
13.2.6  用HTTP/REST API访问远程TF_Serving服务
13.2.7  扩展：关于TF_Serving的更多例子
13.3  实例68：在安卓手机上识别男女
13.3.1  准备工程代码
13.3.2  微调预训练模型
13.3.3  搭建安卓开发环境
13.3.4  制作lite模型文件
13.3.5  修改分类器代码，并运行APP
13.4  实例69：在iPhone手机上识别男女并进行活体检测
13.4.1  搭建iOS开发环境
13.4.2  布署工程代码并编译
13.4.3  载入Lite模型，实现识别男女功能
13.4.4  代码实现：调用摄像头并采集视频流
13.4.5  代码实现：提取人脸特征
13.4.6  活体检测算法介绍
13.4.7  代码实现：实现活体检测算法
13.4.8  代码实现：完成整体功能并运行程序
13.5  实例70：在树莓派上搭建一个目标检测器
13.5.1  安装树莓派系统
13.5.2  在树莓派上安装TensorFlow
13.5.3  编译并安装Protobuf
13.5.4  安装OpenCV
13.5.5  下载目标检测模型SSDLite
13.5.6  代码实现：用SSDLite 模型进行目标检测
第14章
商业实例——科技源于生活，用于生活
14.1  实例71：将特征匹配技术应用在商标识别领域
14.1.1  项目背景
14.1.2  技术方案
14.1.3  预处理图片——统一尺寸
14.1.4  用自编码网络加夹角余弦实现商标识别
14.1.5  用卷积网络加triplet-loss提升特征提取效果
14.1.6  进一步的优化空间
14.2  实例72：用RNN抓取蠕虫病毒
14.2.1  项目背景
14.2.2  判断是否恶意域名不能只靠域名
14.2.3  如何识别恶意域名
14.3  实例73：迎宾机器人的技术关注点——体验优先
14.3.1  迎宾机器人的产品背景
14.3.2  迎宾机器人的实现方案
14.3.3  迎宾机器人的同类产品
14.4  实例74：基于摄像头的路边停车场项目
14.4.1  项目背景
14.4.2  技术方案
14.4.3  方案缺陷
14.4.4  工程化补救方案
14.5  实例75：智能冰箱产品——硬件成本之痛
14.5.1  智能冰箱系列的产品背景
14.5.2  智能冰箱的技术基础
