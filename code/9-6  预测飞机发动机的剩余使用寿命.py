...
yreg = tf.placeholder("float", [None, reg_out])
#
hidden = [100,50,36]
stacked_rnn = []
for i in range(3):
    cell = JANetLSTMCell.JANetLSTMCell(hidden[i], t_max=sequence_length)
    stacked_rnn.append(tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=0.8))
mcell = tf.contrib.rnn.MultiRNNCell(stacked_rnn)

outputs,_  = tf.nn.dynamic_rnn(mcell,x,dtype=tf.float32)
outputs = tf.transpose(outputs, [1, 0, 2])
print(outputs.get_shape())


pred =tf.layers.conv2d(tf.reshape(outputs[-1],[-1,6,6,1]),n_classes,6,activation = tf.nn.relu)
pred =tf.reshape(pred,(-1,n_classes))#分类模型

predreg =tf.layers.conv2d(tf.reshape(outputs[-1],[-1,1,1,36]),1,1,activation = tf.nn.sigmoid)
predreg =tf.reshape(predreg,(-1,1))#回归模型

costreg = tf.reduce_mean(abs(predreg - yreg))
costclass = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(logits=pred, labels=y))

cost =(costreg+costclass)/2
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)


iterator = dataset.make_one_shot_iterator()			#生成一个训练集的迭代器
one_element = iterator.get_next()

iterator_test = testdataset.make_one_shot_iterator()			#生成一个测试集的迭代器
