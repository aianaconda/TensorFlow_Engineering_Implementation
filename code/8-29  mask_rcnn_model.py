...
othernet =  __import__("8-31  othernet")
...
norm_boxes_graph = othernet.norm_boxes_graph
...
USE_RPN_ROIS=True #是否使用RPN结果（不适用时，是调试场景，手动输入）
...
###################################################
        ...
        [detections, mrcnn_class, mrcnn_bbox,mrcnn_mask, rpn_rois, rpn_class, rpn_bbox],#输出
            ...
            if not USE_RPN_ROIS:           #支持手动输入ROI.
                input_rois = KL.Input(shape=[POST_NMS_ROIS_TRAINING, 4], name="input_roi", dtype=np.int32)
                #转为标准坐标
            ...
            #根据输入的样本，制作RPN网络的标签.
            rois, target_class_ids, target_bbox, target_mask =DetectionTargetLayer(self.batch_size, 
            ...
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox = fpn_classifier_graph(rois, mrcnn_feature_maps, input_image_meta,
                                     POOL_SIZE, self.num_class,self.batch_size,train_bn=False,#不用bn
                                     fc_layers_size=1024)#全连接层1024个节点
            #进行语义分割，掩码预测
            mrcnn_mask = build_fpn_mask_graph(rois, mrcnn_feature_maps, input_image_meta,
                                              MASK_POOL_SIZE,self.num_class,self.batch_size, train_bn=False)
            ...
            rpn_bbox_loss = KL.Lambda(lambda x: rpn_bbox_loss_graph(self.batch_size, *x), name="rpn_bbox_loss")(
                [input_rpn_bbox, input_rpn_match, rpn_bbox])
            class_loss = KL.Lambda(lambda x: mrcnn_class_loss_graph(self.num_class,self.batch_size,*x), name="mrcnn_class_loss")(
...
#            from keras.engine import topology as saving
    ...
    def train(self, train_dataset, val_dataset,batch_size, learning_rate, epochs, layers,
        ...
        # Data generators
        train_generator = data_generator(train_dataset,  shuffle=True,augmentation=augmentation,
                                         batch_size=batch_size,
                                         no_augmentation_sources=no_augmentation_sources,num_class = self.num_class)
        val_generator = data_generator(val_dataset, shuffle=True,
                                       batch_size=batch_size,num_class = self.num_class)
        ...
        self.compile(learning_rate, LEARNING_MOMENTUM)#设置模型的优化器及学习参数
...
def data_generator(dataset,  shuffle=True, augment=False, augmentation=None,
    ...
    while True:
        try:
            ...
            image_id = image_ids[image_index]#取一个索引ID
