...
class PyramidROIAlign(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        h = y2 - y1
        w = x2 - x1
        print("ROIboxes",ROIboxes.get_shape())
        print("image_meta",image_meta.get_shape())
        print("h",h.get_shape())#(1, 1000, 1)
        print("w",w.get_shape())

        ###############################在这1000个ROI里，按固定算法匹配到不同level的特征。
        #获得图片形状
        image_shape = parse_image_meta_graph(image_meta)['image_shape'][0]
        print("image_shape",image_shape.get_shape())
        ...
        print("roi",roi_level.get_shape())#(1, 1000)
        ...
        for i, level in enumerate(range(2, 6)):
            ...
            print("ix",level,ix.get_shape(),ix.name)
            print("ix",feature_maps[i].get_shape())

            #在多维上建立索引取值[?,4](828, 4)
            level_boxes = tf.gather_nd(ROIboxes, ix,name="level_boxes")#在(1, 1000, 4)上按照[[0,1],[0,4],,,[0,200]]取值
            print("level_boxes",level_boxes.get_shape())
            # Box indices for crop_and_resize.
            box_indices = tf.cast(ix[:, 0], tf.int32)#(828, )，【0，0，0，0，0，】如果批次为2，就是[000...111]
            print("box_indices",box_indices.get_shape(),box_indices.name)
...
def refine_detections_graph(rois, probs, deltas, window):
    ...
    #取出每个ROI的 Class IDs
    class_ids = tf.argmax(probs, axis=1, output_type=tf.int32)#probs形状为(1000, 81)
...
#    keep = tf.sparse_tensor_to_dense(keep)[0]
    ...
    #拼接输出结果[N, (y1, x1, y2, x2, class_id, score)],N是结果个数
