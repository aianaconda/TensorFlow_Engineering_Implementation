...
def read_item_index_to_entity_id_file():
    file = 'data/' + DATASET + '/item_index2entity_id.txt'#具体的业务条目（item）  索引（entity id）
    print('reading item index to entity id file: ' + file + ' ...')
    i = 0
    for line in open(file, encoding='utf-8').readlines():
        item_index = line.strip().split('\t')[0]
        satori_id = line.strip().split('\t')[1]
        item_index_old2new[item_index] = i#item与行的对应关系（重新编号）
        entity_id2index[satori_id] = i #entity id与行的对应关系（重新编号）
        i += 1

#生成ratings_final.txt  用户 电影  喜欢/不关注（1/0）作为标签
def convert_rating():
    ...
    for line in open(file, encoding='utf-8').readlines()[1:]:
        array = line.strip().split(SEP[DATASET]) #userID	movieID	weight
        ...
        if rating >= THRESHOLD[DATASET]:#用来区分正向，负向
