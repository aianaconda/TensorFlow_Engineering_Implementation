...
below = input_imgs - 2.0/255.0  #定义图片的变化范围
above = input_imgs + 2.0/255.0
...
for i in range(demo_steps):    #训练
    _, loss_value = sess.run([optim_step, loss])
    sess.run(project_step)
#    _,projectedv =sess.run([project_step,projected], feed_dict={x: img})
...
plt.imsave('dog880.jpg',floatarr_to_img(adv))
